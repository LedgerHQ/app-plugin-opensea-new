...
static void debug_tmp(ethQueryContractUI_t *msg, context_t *context) {
...
static void output_item1(ethQueryContractUI_t *msg, context_t *context) {
  ...
  if (context->booleans & ITEM1_FOUND) {  // item1 is found so is NOT ETH
    if (context->booleans & ITEM1_IS_NFT) // item1 is found nft
      snprintf(msg->msg, msg->msgLength, "%d %s",
               U4BE(context->token1_amount, INT256_LENGTH - 4),
               msg->item1->nft.collectionName); // TODO check U4BE() safety
    else                                        // item1 is found erc20
      amountToString(context->token1_amount, INT256_LENGTH,
                     msg->item1->token.decimals, msg->item1->token.ticker,
    ...
    if (context->booleans & ITEM1_IS_NFT) // item1 is unknown nft
      ...
      if (context->booleans & IS_ETH) // if item1 is ETH
        amountToString(context->token1_amount, INT256_LENGTH, DEFAULT_DECIMAL,
                       ETH, msg->msg, msg->msgLength);
      else // item1 is unknown erc20
        amountToString(context->token1_amount, INT256_LENGTH, DEFAULT_DECIMAL,
                       UNKNOWN_ERC20, msg->msg, msg->msgLength);
    }
  }
}
...
static void output_item2(ethQueryContractUI_t *msg, context_t *context) {
  ...
  if (!(context->booleans & ITEM1_FOUND) &&
    ...
    if (context->booleans & ITEM2_IS_NFT)
      ...
      amountToString(context->token2_amount, INT256_LENGTH,
                     msg->item2->token.decimals, msg->item2->token.ticker,
                     msg->msg, msg->msgLength);
    ...
    if (!(context->booleans & ITEM2_IS_NFT)) { // if item2 is money
      if (context->booleans & IS_ETH)
        amountToString(context->token2_amount, INT256_LENGTH, DEFAULT_DECIMAL,
                       ETH, msg->msg, msg->msgLength);
      else // item2 is erc20
        amountToString(context->token2_amount, INT256_LENGTH, DEFAULT_DECIMAL,
                       UNKNOWN_ERC20, msg->msg, msg->msgLength);
...
static void set_send_ui_err(ethQueryContractUI_t *msg, context_t *context) {
  ...
  getEthAddressStringFromBinary((uint8_t *)context->token1_address,
                                (char *)msg->msg + 2, msg->pluginSharedRW->sha3,
                                0);
}
...
static bool get_scroll_direction(uint8_t screen_index,
...
static void get_screen_array(ethQueryContractUI_t *msg, context_t *context) {
  ...
  bool scroll_direction =
      get_scroll_direction(msg->screenIndex, context->previous_screen_index);
